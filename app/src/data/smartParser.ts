...
function parseSOAcknowledgement(text: string): ParsedSO {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const sizeNearby = sizeOnSameLine || sizeOnNextLine || sizeOnNextNextLine || sizeRangePattern;

    if ((sizeOnSameLine || (isItemStart && (sizeOnNextLine || sizeOnNextNextLine || sizeRangePattern)) || (hasCode && sizeNearby)) && INSTRUMENT_KEYWORDS.test(lineClean)) {
  ...
  for (let idx = 0; idx < itemPositions.length; idx++) {
    ...
    if (isGarbageType || !rawType) {
      ...
      const chunkTypeMatch = chunkClean.match(/(Electromagnetic Flowmeter|Vortex Flowmeter|Turbine Flowmeter|Rotameter|Magnetic Level Gauge|Magnetic Level Indicator|Side Mounted Magnetic Level|Level Gauge|Level Indicator|Pressure Transmitter|Sight Glass)/i);
      if (chunkTypeMatch) {
        rawType = chunkTypeMatch[1];
      }
    }
    const detectedFamily = detectProductFamily(rawType + " " + size + " " + chunkText);
    let type = detectedFamily ? getProductLabel(detectedFamily) : rawType;

    // ════════════════════════════════════════════════════════════
    // PERMANENT QUALIFIER PRESERVATION (product-family-agnostic)
    // ════════════════════════════════════════════════════════════
    // SO text often contains qualifying adjectives that are critical
    // for GAD matching in Document Master (e.g. "By-Pass Glass Tube
    // Rotameter" must stay as "By-Pass ... Rotameter", not just
    // "Glass Tube Rotameter").  We detect ANY qualifier that appears
    // in the raw text but is missing from the generic label, and
    // prepend it automatically — no hardcoded family checks needed.
    const QUALIFIER_PREFIXES = [
      ...
      "transparent",                  // Transparent Level Gauge
    ...
    const chunkCleanForQualifier = chunkText.replace(/\s*\|\s*/g, " ").replace(/\s+/g, " ");
    const rawLower = (rawType + " " + chunkCleanForQualifier).toLowerCase();
    const typeLower = type.toLowerCase();
    for (const qualifier of QUALIFIER_PREFIXES) {
      // Check: qualifier appears in raw text AND is missing from the generic label
      if (rawLower.includes(qualifier) && !typeLower.includes(qualifier)) {
        // Capitalise first letter of each word: "by-pass" → "By-Pass"
        const capitalised = qualifier
          .split(/\s+/)
          .map(w => w.charAt(0).toUpperCase() + w.slice(1))
          .join(" ");
        type = capitalised + " " + type;
        break; // Only prepend the first matching qualifier (most specific)
      }
    }
