...
// ─── Tag prefix → product family (most reliable method) ─────
...
export function isInstrumentHeader(text: string): boolean {
  ...
  const n = norm(text);
  // Has a tag number pattern
  if (/\b(EMF|FT|FI|VG|LI)-\d/i.test(text)) return true;
  // Has a size
  if (/\d+\s*(NB|MM)\b/i.test(text) && n.length > 5) return true;
  // Contains instrument keywords
  const keywords = [
    "flowmeter", "flow meter", "rotameter", "level gauge", "level indicator",
